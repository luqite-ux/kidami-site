 ...
 * Converts hardcoded frontend data to Supabase insert format
 * Run after connecting Supabase: node migrate-data.js
 */
...
const products = [
  {
    ...
    amazon_url: "https://www.amazon.com/stores/KIDAMI/page/546B9750-2336-4B85-8F2F-2C57C2997542?utm_source=kidami_site&utm_medium=cta&utm_campaign=brand_portal&utm_content=offroad-6x6-set",
    walmart_url: "https://www.walmart.com/ip/seort/20538824770",
    is_active: true,
    sort_order: 1,
  },
  {
    ...
    amazon_url: "https://www.amazon.com/stores/KIDAMI/page/546B9750-2336-4B85-8F2F-2C57C2997542?utm_source=kidami_site&utm_medium=cta&utm_campaign=brand_portal&utm_content=city-heroes-collection",
    walmart_url: "https://www.walmart.com/ip/seort/20538824770",
    is_active: true,
    sort_order: 2,
  },
  {
    ...
    amazon_url: "https://www.amazon.com/stores/KIDAMI/page/546B9750-2336-4B85-8F2F-2C57C2997542?utm_source=kidami_site&utm_medium=cta&utm_campaign=brand_portal&utm_content=classic-5-pack",
    walmart_url: "https://www.walmart.com/ip/seort/20538824770",
    is_active: true,
    sort_order: 3,
  },
  {
    ...
    amazon_url: "https://www.amazon.com/stores/KIDAMI/page/546B9750-2336-4B85-8F2F-2C57C2997542?utm_source=kidami_site&utm_medium=cta&utm_campaign=brand_portal&utm_content=magnetic-ludo",
    walmart_url: "https://www.walmart.com/ip/seort/20538824770",
    is_active: true,
    sort_order: 4,
  },
  {
    ...
    amazon_url: "https://www.amazon.com/stores/KIDAMI/page/546B9750-2336-4B85-8F2F-2C57C2997542?utm_source=kidami_site&utm_medium=cta&utm_campaign=brand_portal&utm_content=snakes-and-ladders",
    walmart_url: "https://www.walmart.com/ip/seort/20538824770",
    is_active: true,
    sort_order: 5,
  },
...
const reviews = [
  ...
  },
  {
    ...
    product_name: "City Heroes Collection",
    is_approved: true,
  },
  {
    name: "Priya K.",
    ...
    product_name: "Magnetic Travel Ludo",
    is_approved: true,
  },
  {
    name: "Tom & Elena W.",
...
const seoSettings = [
  ...
  },
  {
    ...
    description: "Questions about KIDAMI toys? Read our FAQ on safety, shipping and replacements — or send us feedback.",
    keywords: ["KIDAMI contact", "customer service", "toy support"],
    priority: 0.5,
    changefreq: "monthly",
  },
  {
    page_path: "/learn",
    lang_code: "en",
    title: "STEM & Learning — Play Ideas Backed by Child-Development Science",
    description: "Articles and guides on play-based learning: how die-cast cars build fine motor skills, why board games teach logic and probability.",
    keywords: ["STEM toys", "learning toys", "child development"],
    priority: 0.7,
    changefreq: "weekly",
  },
  {
    page_path: "/explore",
    lang_code: "en",
    title: "Inspiration Library — Shop Toys by Play Skill",
    description: "Explore KIDAMI toys by what they teach: hands-on skills, outdoor adventure, early learning and family time.",
    keywords: ["toy inspiration", "play skills", "educational toys"],
    priority: 0.7,
    changefreq: "weekly",
  },
];

console.log("=== KIDAMI Data Migration ===\n");
console.log("Products:", products.length);
console.log("Articles:", articles.length);
console.log("Reviews:", reviews.length);
console.log("SEO Settings:", seoSettings.length);
console.log("\nTo migrate, run the following SQL in Supabase SQL Editor:");
console.log("1. Execute supabase/schema.sql");
console.log("2. Use supabase-js to insert the data above");
console.log("\nOr use this script with your SERVICE ROLE KEY:");
console.log(`
const { createClient } = require('@supabase/supabase-js');
const supabase = createClient('https://yklxtfilguqyfrxzfgzh.supabase.co', 'YOUR_SERVICE_ROLE_KEY');

async function migrate() {
  // Insert products
  const { data: pData, error: pError } = await supabase.from('products').insert(products);
  if (pError) console.error('Products error:', pError);
  else console.log('Products inserted:', pData?.length);

  // Insert articles
  const { data: aData, error: aError } = await supabase.from('articles').insert(articles);
  if (aError) console.error('Articles error:', aError);
  else console.log('Articles inserted:', aData?.length);

  // Insert reviews
  const { data: rData, error: rError } = await supabase.from('reviews').insert(reviews);
  if (rError) console.error('Reviews error:', rError);
  else console.log('Reviews inserted:', rData?.length);

  // Insert SEO
  const { data: sData, error: sError } = await supabase.from('seo_settings').insert(seoSettings);
  if (sError) console.error('SEO error:', sError);
  else console.log('SEO inserted:', sData?.length);
}
...
module.exports = { products, articles, reviews, seoSettings };
